...
@section('dashboard-content')
    <div x-data="{
        ...
        bookingForm: { tamu: '', mode: 'paket', durasi: 1, selectedPackage: null }, // mode: 'paket' or 'open'
        ...
        executeStartSession(shouldPrint) {
            ...
            this.showNotification('Sesi dimulai' + (shouldPrint ? ' & struk dicetak!' : '!'), 'success');
        },
            ...
            const printContent = `
                <html>
                    ...
                    <body>
                        ...
                        <div class='time-highlight'>${data.masuk} → ${data.keluar}</div>
    ...
    <template x-teleport="body">
        <div x-show="showToast" 
            ...
            <div>
                <h4 class="text-sm font-bold uppercase tracking-wider" x-text="toastType === 'success' ? 'Berhasil' : (toastType === 'error' ? 'Gagal' : 'Pemberitahuan')"></h4>
        ...
        <div x-show="rooms.some(r => r.status === 'Digunakan')" class="bg-[#080808] border border-white/5 rounded-2xl overflow-hidden" x-transition>
            <div class="px-8 py-6 border-b border-white/5 bg-black/20 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <div class="relative">
                        <span class="absolute -top-0.5 -right-0.5 w-2 h-2 bg-[#D0B75B] rounded-full animate-pulse"></span>
                        <i data-lucide="mic-2" class="w-5 h-5 text-[#D0B75B]"></i>
                    </div>
                    <h2 class="text-sm font-bold text-gray-200 uppercase tracking-widest" style="font-family: 'Inter';">Sesi Aktif (Live Sessions)</h2>
                </div>
                <div class="flex items-center gap-2 px-3 py-1 bg-[#D0B75B]/10 rounded-full border border-[#D0B75B]/20">
                     <template x-if="rooms.filter(r => r.status === 'Digunakan').length > 0">
                        <span class="text-[10px] font-black text-[#D0B75B] uppercase tracking-widest" x-text="rooms.filter(r => r.status === 'Digunakan').length + ' Active'"></span>
                     </template>
                </div>
            </div>
                ...
                <table class="w-full text-xs" style="font-family: 'Inter';">
                    <thead>
                        <tr class="text-gray-600 text-[9px] font-black uppercase tracking-[0.25em] bg-black/40">
                            <th class="text-left px-6 py-3">Room Information</th>
                            <th class="text-left px-6 py-3">Guest Name</th>
                            <th class="text-center px-6 py-3">Billing Mode</th>
                            <th class="text-center px-6 py-3">Check In</th>
                            <th class="text-center px-6 py-3">Duration/Timer</th>
                            <th class="text-center px-6 py-3">Current Cost</th>
                            ...
                            <template x-if="room.status === 'Digunakan'">
                                <tr class="hover:bg-white/[0.01] transition-colors group">
                                    <td class="px-6 py-3">
                                        <div class="flex items-center gap-3">
                                            <div>
                                                <span class="text-white font-black text-sm block" x-text="room.nama"></span>
                                                <span class="text-[9px] text-gray-500 uppercase tracking-widest" x-text="room.tipe"></span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-3">
                                        <span class="text-white font-bold" x-text="room.tamu"></span>
                                    </td>
                                    <td class="px-6 py-3 text-center">
                                        <span class="inline-flex items-center justify-center px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest border whitespace-nowrap"
                                              :class="room.billing_mode === 'paket' ? 'bg-[#D0B75B]/20 text-[#D0B75B] border-[#D0B75B]/20' : 'bg-blue-500/10 text-blue-500 border-blue-500/20'"
                                              x-text="room.billing_mode === 'paket' ? 'Paket ' + room.booking_duration + ' Jam' : 'Open Billing'">
                                        </span>
                                    </td>
                                    <td class="px-6 py-3 text-center text-gray-400 font-mono">
                                        <span x-text="new Date(room.booking_start).toLocaleTimeString('id-ID', {hour: '2-digit', minute:'2-digit'})"></span>
                                    </td>
                                    <td class="px-6 py-3 text-center">
                                        <div class="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg bg-black border border-white/10 font-mono font-bold text-xs"
                                             :class="room.billing_mode === 'paket' ? (room.hampir_habis ? 'text-red-500 animate-pulse border-red-500/30' : 'text-white') : 'text-blue-500 border-blue-500/30'">
                                             <i data-lucide="timer" class="w-3 h-3 opacity-50"></i>
                                             <span x-text="room.billing_mode === 'paket' ? formatTimeArray(room.sisa_detik) : formatTimeArray(room.durasi_berjalan)"></span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-3 text-center">
                                        <span class="text-[#D0B75B] font-mono font-bold" x-text="calculateCurrentPrice(room)"></span>
                                    </td>
                                    <td class="px-6 py-3 text-center">
                                        <button @click="openControl(room)" class="bg-red-500 hover:bg-red-600 text-white text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-lg transition-all">
                                            Checkout
                                        </button>
                                    </td>
                                </tr>
                        ...
                        <div x-show="selectedRoom.status === 'Digunakan'" class="p-6 space-y-6">
                            <div class="flex items-center justify-between p-4 bg-zinc-900/50 rounded-xl border border-white/5">
                                ...
                                <div class="text-right">
                                    ...
                                    <p class="text-[#D0B75B] font-mono font-bold text-lg" 
